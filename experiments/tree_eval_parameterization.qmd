...
## Tree matching eval
This has experiments for the quality of matching based on the edge buffer distance, by evaluating the quality of matches only on the outer 6 meters of the plot.
...
eval_preds = function(
  ...
  if (nrow(pred_trees) > 0) {
    ...
    if (TRUE) {
      obs_bounds_center = st_buffer(obs_bounds, -6)
      obs_bounds_center = st_transform(obs_bounds_center, st_crs(pred_trees))
      pred_trees_in_center = st_intersects(pred_trees, obs_bounds_center, sparse = FALSE)
      obs_trees_matched_in_center = st_intersects(obs_trees_matched, obs_bounds_center, sparse = FALSE)

      pred_trees$pred_tree_core_area[pred_trees_in_center] = FALSE
      obs_trees_matched$obs_tree_core_area[obs_trees_matched_in_center] = FALSE
    }

    # Determine which trees are within the outer ring
    # * This can be done by updating pred_tree_core_area such that it only includes ones in the edge
    # Compute the match statistics
    match_stats = ofo::compute_match_stats(
      pred_trees,
      obs_trees_matched,
...
compute_plot_metrics = function(
  ...
  obs_trees = st_read(reference_file, quiet = TRUE)
  pred_trees = st_read(preds_file, quiet = TRUE)
  obs_bounds = st_read(plot_bounds, quiet = TRUE)
...
```{r}
across_plots_experiments("edge_buffer", 0:10 / 2)
```
...
```{r}
# across_plots_experiments("min_height_pred", 0:10)
```

```{r}
# across_plots_experiments("min_height_obs", 0:10)
```